...
import { MajorsInsights, EmploymentTimelineData } from "@/app/majors/ty";
...
interface OccupationWithMetrics {
  name: string;
  metrics: {
    graduates: number;
    employmentRate: number;
...
//   return data
//     .sort(
//       (a: GenderDistribution, b: GenderDistribution) =>
//         b.male.count + b.female.count - (a.male.count + a.female.count)
//     )
//     .slice(0, 3);
// };
...
const GenderDistributionModal = ({
  ...
  return (
    <div
      className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex justify-center"
      onClick={onClose}
    >
      <div
        style={modalStyle}
            ...
            .map((item, index) => (
              ...
              >
                <div className="bg-gradient-to-r from-[#1a2657] to-[#1a2657]/90 p-6 rounded-lg">
                  <h3 className="text-white/80 text-sm mb-3 truncate">
                    {item.generalMajor}
                  </h3>
...
function MajorsPageContent() {
  ...
  const generalMajors = mockData.majorsInsights.overall.basicMetrics.map(
    (item: { generalMajor: string }) => item.generalMajor
  );
  ...
  const getTopNarrowMajorsData = (
    generalMajor: string | null,
    narrowMajor: string | null
  ) => {
    if (!generalMajor) {
      // Case 1: All General Majors - use majorsInsights/overall/basicMetrics
      return mockData.majorsInsights.overall.basicMetrics
        .sort(
          (a: { graduates: number }, b: { graduates: number }) =>
            b.graduates - a.graduates
        )
        .slice(0, 5)
        .map(
          (item: {
    ...
    const generalMajorData =
      mockData.majorsInsights.byGeneralMajor.generalMajors.find(
        (major: { name: string }) => major.name === generalMajor
      );
    ...
    const narrowMajorData = generalMajorData.byNarrowMajor?.narrowMajors.find(
      (item: { name: string; overall: GeneralMajorData }) =>
        item.name === narrowMajor
    );
    ...
    return narrowMajorData.overall.basicMetrics
      ...
      .slice(0, 5)
      .map((item: { name?: string; graduates: number }) => ({
  ...
  const getTopEmployableNarrowMajorsData = (
    generalMajor: string | null,
    narrowMajor: string | null
  ) => {
    if (!generalMajor) {
      ...
      return mockData.majorsInsights.overall.basicMetrics
        .sort(
          (a: { employmentRate: number }, b: { employmentRate: number }) =>
            b.employmentRate - a.employmentRate
        )
        .slice(0, 5)
        .map(
          (item: {
    ...
    return narrowMajorData.overall.basicMetrics
      ...
      .slice(0, 5)
      .map((item: { name?: string; employmentRate: number }) => ({
        narrowMajor: item.name || "",
        value: item.employmentRate,
      }));
  };
  ...
  const getTopPaidNarrowMajorsData = (
    generalMajor: string | null,
    narrowMajor: string | null
  ) => {
    if (!generalMajor) {
      ...
      return mockData.majorsInsights.overall.basicMetrics
        .sort(
          (a: { averageSalary: number }, b: { averageSalary: number }) =>
            b.averageSalary - a.averageSalary
        )
        .slice(0, 5)
        .map(
          (item: {
    ...
    return narrowMajorData.overall.basicMetrics
      ...
      .slice(0, 5)
      .map((item: { name?: string; averageSalary: number }) => ({
        narrowMajor: item.name || "",
        value: item.averageSalary,
      }));
  };
  ...
  const getTopMajorsByPopularOccupationsData = (
    generalMajor: string | null,
    narrowMajor: string | null
  ) => {
    if (!generalMajor) {
      ...
      const occupations =
        mockData.majorsInsights.overall.topMajorsByOccupation?.occupations ||
        [];
      const totalGraduates = occupations.reduce(
        (sum: number, curr: OccupationWithMetrics) =>
          sum + curr.metrics.graduates,
        0
      );

      return occupations.map((occupation: OccupationWithMetrics) => ({
    ...
    if (!narrowMajor) {
      ...
      const occupations =
        generalMajorData.overall.topMajorsByOccupation?.occupations || [];
      const totalGraduates = occupations.reduce(
        (sum: number, curr: OccupationWithMetrics) =>
          sum + curr.metrics.graduates,
        0
    ...
    if (!narrowMajorData?.overall?.topMajorsByOccupation?.occupations)
      return [];

    const occupations =
      narrowMajorData.overall.topMajorsByOccupation.occupations;
    const totalGraduates = occupations.reduce(
      (sum: number, curr: OccupationWithMetrics) =>
        sum + curr.metrics.graduates,
      0
  ...
  const getTopMajorsByMostEmployedOccupation = (
    generalMajor: string | null,
    narrowMajor: string | null
  ) => {
    if (!generalMajor) {
      ...
      return occupations
        .sort(
          (a: Occupation, b: Occupation) =>
            b.metrics.employmentRate - a.metrics.employmentRate
        )
        .slice(0, 5)
        .map(
          (occupation: {
            name: string;
            metrics: { employmentRate: number };
          }) => ({
            major: occupation.name,
            employmentRate: occupation.metrics.employmentRate,
  ...
  const getTopMajorsByHighestPaidOccupation = (
    generalMajor: string | null,
    narrowMajor: string | null
  ) => {
    if (!generalMajor) {
      ...
      return occupations
        .sort(
          (a: Occupation, b: Occupation) =>
            b.metrics.averageSalary - a.metrics.averageSalary
        )
        .slice(0, 5)
        .map((item: Occupation) => ({
          narrowMajor: item.name,
          value: item.metrics.averageSalary,
        }));
    }
  ...
  const getGenderDistributionData = () => {
    ...
    if (!generalMajor) {
      return mockData.majorsInsights.overall.basicMetrics.map((item) => ({
        ...
        male: { count: item.maleCount, percentage: item.malePercentage },
        female: { count: item.femaleCount, percentage: item.femalePercentage },
      }));
    }

    const overallGenderData =
      mockData.majorsInsights.overall.genderDistribution;
    ...
    const narrowMajorData = narrowMajorDataList.find(
      (major: { narrowMajor?: string }) => major.narrowMajor === narrowMajor
    );
  ...
  const getSalaryDistributionData = (): SalaryDistribution[] => {
    ...
    // Case 3: Both general and narrow major selected
    const narrowMajorData = generalMajorData.byNarrowMajor?.narrowMajors.find(
      (major: { name: string }) => major.name === narrowMajor
    );
  ...
  const getEmploymentTimingData = (
    filters: MajorFilterOptions
  ): EmploymentTiming[] => {
    const { generalMajor, narrowMajor } = filters;
    ...
    // Case 2: General major selected, but no narrow major
    if (!narrowMajor) {
      return generalMajorData.overall.employmentTiming.map(
        (item: {
          narrowMajor: string;
          beforeGraduation: string | number;
          withinYear: string | number;
          afterYear: string | number;
        }) => ({
          ...item,
  ...
  const getEmploymentTimelineData = (
    filters: MajorFilterOptions
  ): EmploymentTimelineData[] => {
    const { generalMajor, narrowMajor } = filters;
    ...
    // Case 2: General major selected, but no narrow major
    if (!narrowMajor) {
      return generalMajorData.overall.employmentTimeline.map(
        (item: { name: string; period: string; percentage: number }) => ({
          ...item,
    ...
    return narrowMajorData.overall.employmentTimeline.map(
      (item: { name: string; period: string; percentage: number }) => ({
        ...item,
        generalMajor: item.name || narrowMajor,
      })
  ...
  const getTopMajorsGenderGapData = () => {
    return mockData.majorsInsights.overall.topGeneralMajorsInsights.byGenderGap.rankings.rankings.map(
      (item) => ({
        generalMajor: item.generalMajor,
  ...
  const getTopOccupationsGenderGapData = (filters: MajorFilterOptions) => {
    ...
    if (!generalMajor) {
      return (
        mockData.majorsInsights.overall.topMajorsByOccupation?.occupations.map(
          (occupation: {
            name: string;
            metrics: {
              genderDistribution?: {
            ...
            malePercentage:
              occupation.metrics.genderDistribution?.male?.percentage || 0,
            femalePercentage:
    ...
    if (!narrowMajor) {
      return (
        selectedGeneralMajor.overall.topMajorsByOccupation?.occupations.map(
          (occupation: {
            name: string;
            metrics: {
              genderDistribution?: {
    ...
    const selectedNarrowMajor =
      selectedGeneralMajor.byNarrowMajor?.narrowMajors.find(
        (major: { name: string }) => major.name === narrowMajor
      );
    if (!selectedNarrowMajor) return [];

    return selectedNarrowMajor.overall.topMajorsByOccupation?.occupations.map(
      (occupation: {
        name: string;
        metrics: {
          genderDistribution?: {
  ...
  const getTopMajorsGenderGap = (filters: MajorFilterOptions) => {
    ...
    if (!generalMajor) {
      ...
      return mockData.majorsInsights.overall.topGeneralMajorsInsights
        .byGenderGap.rankings.rankings;
      ...
      const selectedNarrowMajor =
        selectedMajor?.byNarrowMajor?.narrowMajors.find(
          (major: { name: string }) => major.name === narrowMajor
        );
      const rankings =
        selectedNarrowMajor?.overall.topMajorsInsights?.byGenderGap?.rankings ||
        [];
      return rankings.map((item: { name: string; genderGap: number }) => ({
        ...item,
  ...
  const getSummaryCardData = () => {
    ...
    if (!narrowMajor) {
      ...
      const selectedMajorMetrics =
        mockData.majorsInsights.overall.basicMetrics.find(
          (major) => major.generalMajor === generalMajor
        );
    ...
    const selectedNarrowMajorMetrics =
      generalMajorData.overall.basicMetrics.find(
        (major: { narrowMajor: string }) => major.narrowMajor === narrowMajor
      );
    ...
    <div className="space-y-6 p-4">
      <div className="mb-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className={`${fontClasses.heading} text-white text-sm`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 ">
        ...
        <div className="bg-gradient-to-r from-[#1a2657] to-[#1a2657]/90 p-6 rounded-lg">
          ...
          <div className="text-white text-2xl font-semibold">
            {formatCurrency(getSummaryCardData()?.averageSalary).replace(
              "$",
              "SAR "
            )}
          </div>
      ...
      {/* {!filters.generalMajor && (
        <div className="flex justify-between items-center mb-4">
          <h2 className={`${fontClasses.heading} text-white text-xl mb-4`}>
            Gender Distribution By All Majors
          </h2>
          <button
          ...
          <div className="grid grid-cols-3 gap-4 w-full mb-8">
            {getTopThreeByGraduates(getGenderDistributionData())
              .filter(
                (data) =>
                  data?.generalMajor &&
                  (data?.male?.count > 0 || data?.female?.count > 0)
              ...
              .map((data, index) => (
                <div
                  ...
                  className="cursor-pointer"
                  onClick={() =>
                    handleGeneralMajorChange(data.generalMajor || "")
                  }
                >
                  <div className="bg-gradient-to-r from-[#1a2657] to-[#1a2657]/90 p-6 rounded-lg">
                    <h3
                      className={`${fontClasses.subheading} text-white/80 text-sm mb-3 truncate`}
                    >
                      {data.generalMajor}
                    </h3>
                    <AllMajorsGenderDistributionChart
      ...
      <div id="general-majors-insights" className="mt-8">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-4 w-full">
              ...
              <TopEmployableNarrowMajorsChart
                generalMajor={filters.generalMajor}
                narrowMajor={filters.narrowMajor}
                data={getTopEmployableNarrowMajorsData(
                  filters.generalMajor,
                  filters.narrowMajor
                )}
                onBarClick={(narrowMajor) => {
              ...
              <TopPaidNarrowMajorsChart
                generalMajor={filters.generalMajor}
                narrowMajor={filters.narrowMajor}
                data={getTopPaidNarrowMajorsData(
                  filters.generalMajor,
                  filters.narrowMajor
                )}
                onBarClick={(narrowMajor) => {
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-4 w-full">
              <TopMajorsByPopularOccupationsChart
                generalMajor={filters.generalMajor}
                narrowMajor={filters.narrowMajor}
                data={getTopMajorsByPopularOccupationsData(
                  filters.generalMajor,
                  filters.narrowMajor
                )}
              />
              ...
              <TopMajorsByMostEmployedOccupationChart
                generalMajor={filters.generalMajor}
                data={getTopMajorsByMostEmployedOccupation(
                  filters.generalMajor,
                  filters.narrowMajor
                )}
              />
              <TopMajorsByHighestPaidOccupationChart
                generalMajor={filters.generalMajor}
                data={getTopMajorsByHighestPaidOccupation(
                  filters.generalMajor,
                  filters.narrowMajor
                )}
              />
        ...
        <div id="timing-insights" className="mt-8">
          <div className="grid grid-cols-2 gap-4 w-full">
            <div>
              <h3
                className={`${fontClasses.subheading} text-white text-lg mb-4`}
              >
