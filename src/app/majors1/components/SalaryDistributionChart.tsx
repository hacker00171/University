...
export default function SalaryDistributionChart({
  ...
  const handleBarClick = (data: BarClickData) => {
    const item = data?.payload?.originalData;
    if (!item) return;
    ...
    <div className="h-[400px] bg-gradient-to-r from-[#1a2657] to-[#1a2657]/90 p-6 rounded-lg">
      <h3 className="text-white text-lg mb-4">
        {generalMajor
          ? `Salary Distribution for ${
              data[0]?.narrowMajor || data[0]?.generalMajor || generalMajor
            }`
      ...
      <ResponsiveContainer width="100%" height="90%">
        ...
        >
          ...
          <ReferenceLine
            x={chartData[0]?.median || undefined}
            stroke="#2AB1BB"
            strokeWidth={2}
            label={{
              value: chartData[0]?.median
                ? `Median: SAR ${chartData[0].median.toLocaleString()}`
                : undefined,
              fill: "#2AB1BB",
              position: "top",
              fontSize: 12,
