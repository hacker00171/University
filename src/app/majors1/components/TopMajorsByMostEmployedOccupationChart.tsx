...
export default function TopMajorsByMostEmployedOccupationChart({ data }: TopMajorsByMostEmployedOccupationChartProps) {
  ...
  const chartData = data
    ...
    .map(item => ({
      ...
      "Employment Rate": item.employmentRate,
    }))
    .reverse();

  return (
    <div className="h-[400px] bg-gradient-to-r from-[#1a2657] to-[#1a2657]/90 p-6 rounded-lg">
      <h3 className="text-lg font-medium text-gray-200 mb-4">Most Employed Occupations</h3>
      <div style={{ height: '80%' }}>
        <ResponsiveBar
          data={chartData}
          keys={["Employment Rate"]}
          indexBy="name"
          layout="horizontal"
