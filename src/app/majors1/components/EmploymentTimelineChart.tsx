...
export default function EmploymentTimelineChart({ 
  ...
  const transformedData: ChartData[] = data.map((item) => ({
    ...
    "Before Graduation": item.waitingPeriods?.beforeGraduation?.percentage || 0,
    "Within First Year": item.waitingPeriods?.withinYear?.percentage || 0,
    "After First Year": item.waitingPeriods?.afterYear?.percentage || 0,
