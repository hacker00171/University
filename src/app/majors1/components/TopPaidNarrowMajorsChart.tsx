  ...
  return (
    <div className="h-[400px] bg-gradient-to-r from-[#1a2657] to-[#1a2657]/90 p-6 rounded-lg">
      <h3 className="text-white text-lg mb-4">
        {!generalMajor 
          ? 'Top General Majors by Average Salary'
          : narrowMajor 
            ? `Highest Paying Majors in ${narrowMajor}`
            : `Highest Paying Narrow Majors in ${generalMajor}`
        }
      </h3>
        ...
        >
          <defs>
            <linearGradient id="salaryGradient" x1="1" y1="0" x2="0" y2="0">
              <stop offset="0%" stopColor="#266bfc" stopOpacity={1} />
              <stop offset="100%" stopColor="#266bfc" stopOpacity={0.6} />
            </linearGradient>
          </defs>
          ...
          <Tooltip
            ...
            formatter={(value: number) => [`SAR ${value.toLocaleString()}`, 'Average Salary']}
          />
          <Bar 
            dataKey="value" 
