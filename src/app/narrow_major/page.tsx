...
export default function ThirdPage() {
  ...
  const generalMajorData = mockDataMajor.majorsInsights.byGeneralMajor.generalMajors.find(
    major => major.generalMajor.toLowerCase() === selectedGeneralMajor?.toLowerCase()
  );


  // Find the narrow major data
  const narrowMajorData = generalMajorData?.byNarrowMajor.narrowMajors.find(
    major => major.narrowMajor.toLowerCase() === selectedNarrowMajor?.toLowerCase()
  );
    ...
    <div className="min-h-screen bg-transparent backdrop-blur-sm">
      ...
      <div className="relative z-10 container mx-auto px-6 py-8 max-w-7xl">
        ...
        <div className="mb-6">
          ...
          <h1  className="text-white/90 font-['Neo_Sans_Medium'] text-2xl text-center">{selectedNarrowMajor}</h1 >
        </div>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1">
                <div className="space-y-1.5">
                  ...
                  >
                    <span className="text-sm text-white/50 ml-1">before graduate</span>
                    <span className="text-lg font-['Neo_Sans_Bold'] font-bold text-white">
                      {narrowMajorData?.overall.totalMetrics.timeToEmployment.beforeGraduation.percentage}%
                    </span>
                  </div>
                  ...
                  >
                    ...
                    <span className="text-lg font-['Neo_Sans_Bold'] font-bold text-white">
                      {narrowMajorData?.overall.totalMetrics.timeToEmployment.withinFirstYear.percentage}%
                    </span>
                  </div>
                  ...
                  >
                    ...
                    <span className="text-lg font-['Neo_Sans_Bold'] font-bold text-white">
                      {narrowMajorData?.overall.totalMetrics.timeToEmployment.afterFirstYear.percentage}%
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-3">
          {/* Top 5 Occupations */}
          <div className="lg:col-span-1 bg-[#1E1F5E]/50 rounded-2xl p-6 backdrop-blur-sm border hover:border-white transition-colors">
                    ...
                    <div className="relative flex-1 h-8">
                      ...
                      >
                        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center">
                          <span className="text-sm font-['Neo_Sans_Bold'] font-bold text-white">{occupation.averageSalary.toLocaleString()}</span>
                          <span className="text-xs font-['Roboto_Regular'] font-normal text-white ml-1">SAR</span>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="bg-[#1E1F5E]/50 rounded-2xl p-6 backdrop-blur-sm border hover:border-white transition-colors">
            <h2 className="text-xl font-['Neo_Sans_Bold'] mb-6 flex items-center gap-2">
              <div className="bg-[#2CCAD3]/10 p-1.5 rounded-lg">
                <Image src="/icons/degree.svg" alt="Education" width={24} height={24} />
              </div>
            ...
            <div className="space-y-4 relative">
              ...
              {overviewStats?.educationLevelInsights.map((level, index) => {
                const maxGraduates = Math.max(...overviewStats.educationLevelInsights.map(l => l.totalGraduates));
